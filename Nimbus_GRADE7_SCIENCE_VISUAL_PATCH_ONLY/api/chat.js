...
async function requestScienceRag(apiKey, model, userText) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    parts.push({ text: userText || (requestedAttachment?.name ? `Please analyse the attached file "${requestedAttachment.name}" and help the student.` : 'Hello!') });

    const chain = MODEL_CHAINS[body?.model] || MODEL_CHAINS.ror;

    if (scienceQuery && !requestedAttachment) {
      for (const ragModel of chain) {
        const rag = await requestScienceRag(apiKey, ragModel, userText);
        if (rag?.text) {
          const parsedText = beaconhouseQuery ? `${rag.text}\n${BEACONHOUSE_CONTEXT}` : rag.text;
          return res.status(200).json({
            reply: cleanNimbusText(parsedText) + sourceNote(rag.sources),
            visual: rag.visual,
            sources: rag.sources,
    ...
    for (const model of chain) {
      try {
        ...
        if (response.ok) {
          ...
          return res.status(200).json({ reply: cleanNimbusText(parsed.text), visual: parsed.visual, model: body?.model || 'ror', limit: Number(process.env.NIMBUS_DAILY_LIMIT || 1500), grounded: false });
        }
