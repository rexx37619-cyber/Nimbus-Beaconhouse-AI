...
const LIMIT = Number.isFinite(Number(globalThis.NIMBUS_LIMIT_PLACEHOLDER)) ? Number(globalThis.NIMBUS_LIMIT_PLACEHOLDER) : 1500;
...
BEACONHOUSE:
Use official/public Beaconhouse sources when specific current campus or programme information is needed.
...
function cleanId(value) { return String(value || 'anonymous').trim().slice(0, 200) || 'anonymous'; }
...
export async function onRequestPost(context) {
  ...
  if (!apiKey) return Response.json({ reply: 'Nimbus is temporarily busy. Please try again in a moment.' }, { status: 200 });
  try {
    ...
    for (const model of chain) {
      try {
        ...
        if (response.ok) {
          ...
          return Response.json({ reply: parsed.text, visual: parsed.visual, model: body?.model || 'ror', limit: Number(context.env.NIMBUS_DAILY_LIMIT || 1500) });
        }
    ...
    return Response.json({ reply: 'Nimbus is temporarily busy. Please try again in a moment.' }, { status: 200 });
  } catch (e) {
