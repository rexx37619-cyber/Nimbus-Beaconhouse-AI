...
async function generateCloudflare(accountId,token,prompt){
  ...
  // Cloudflare FLUX currently limits prompts to 2048 characters. Keep the style instructions
  // and the student-specific topic while staying safely below the limit.
  const rawPrompt=String(imagePrompt(prompt)||'').replace(/\s+/g,' ').trim();
  const cfPrompt=rawPrompt.length<=1800?rawPrompt:`${rawPrompt.slice(0,420)} Topic/details: ${rawPrompt.slice(-1300)}`.slice(0,1800);
  // Match the known-good REST payload shape: prompt + steps. Avoid extra fields that can
  // trigger provider-specific validation differences.
  const payload={prompt:cfPrompt,steps:4};
  ...
  if(!r.ok) throw new Error(data?.errors?.[0]?.message||data?.messages?.[0]?.message||`Cloudflare image request failed (${r.status})`);
